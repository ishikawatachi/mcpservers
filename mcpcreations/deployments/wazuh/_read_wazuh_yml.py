#!/usr/bin/env python3
"""Read current wazuh.yml, then overwrite with correct API endpoint, restart dashboard."""
...
def run_cmd(base, tok, cmd, bind):
    ...
    resp = docker(base, tok, "POST", f"containers/create?name={name}", {
        ...
        "HostConfig": {"Binds": [bind], "AutoRemove": False}
    })
    cid = resp["Id"]
    ...
    for _ in range(20):
        ...
        if info.get("State",{}).get("Status") == "exited":
            break
    r = urllib.request.Request(f"{base}/api/endpoints/{EP}/docker/containers/{cid}/logs?stdout=1&stderr=1&tail=50",
...
BIND = "/volume1/docker/wazuh/wazuh-dashboard:/t"

# Step 1: read current wazuh.yml
print("=== Current wazuh.yml ===")
result = run_cmd(base, tok, "cat /t/wazuh/config/wazuh.yml 2>&1 || echo '<not found>'", BIND)
print(result)

# Step 2: write correct wazuh.yml
# API: https://wazuh.manager:55000, user: wazuh-wui, pass from DASHBOARD_PASSWORD
CORRECT_YML = """hosts:
  - default:
      ...
      run_as: false
"""
b64 = base64.b64encode(CORRECT_YML.encode()).decode()
write_cmd = (
    f"mkdir -p /t/wazuh/config && "
    f"echo '{b64}' | base64 -d > /t/wazuh/config/wazuh.yml && "
    f"chown 1000:1000 /t/wazuh/config/wazuh.yml && "
    f"echo 'Written:' && cat /t/wazuh/config/wazuh.yml"
)
print("=== Writing correct wazuh.yml ===")
result = run_cmd(base, tok, write_cmd, BIND)
print(result)
...
if dash:
    ...
    print("Restarted. Wait ~30s then reload the page.")
else:
